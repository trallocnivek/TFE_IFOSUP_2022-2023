...
public function setLogo($aUrl='', $aWidth=30, $aX=10, $aY=6, $type='logo'){
	if($type==='logo'){
		$this->logoUrl = $aUrl;
		$this->logoPosX = $aX;
		$this->logoPosY = $aY;
		...
		$this->barcodeUrl = $aUrl;
		$this->barcodePosX = $aX;
		$this->barcodePosY = $aY;
		$this->barcodeWidth = $aWidth;
	}
}
// - - - - - - - - - - - - - - - - - - - - - - - - -
...
function Header(){
	...
	if(!empty($this->barcodeUrl)){
		$this->Image($this->barcodeUrl, $this->barcodePosX, $this->barcodePosY, $this->barcodeWidth);
		$this->Ln(12);
	}
	// elements d'entete
